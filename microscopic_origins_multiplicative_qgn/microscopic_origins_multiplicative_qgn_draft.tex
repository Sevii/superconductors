...
\fancyhead[R]{\small Second review-integrated draft -- July 2026}
\fancyfoot[C]{\thepage}
...
\date{Second review-integrated research draft, July 26, 2026}
...
\item In the unscreened molecular-shell family, an order-$\lambda^4$ diagonal charge selector accompanies the order-$\lambda^6$ multiplicative mixing term.
...
\item A screened periodic array produces $\sum_xK_x^\dagger K_x$ at order $\lambda^6$ with the first connected overlap corrections delayed to order $\lambda^8$.  The order-four cancellation must be accurate to $O(\lambda^2\bar\alpha)$ at fixed $\lambda$; without that matching, the generic unscreened array has an explicit order-$\lambda^4$ diagonal selector followed by order-$\lambda^6$ composition mixing.
\end{enumerate}
...
For $L\ge2$ and the product AGP $\ket{n_1,n_2}$, $H_{\rm ch}$ is diagonal in composition space and
\begin{equation}
\boxed{
\begin{aligned}
...
The nonconstant term is a positive diagonal selector favoring block-polarized compositions.  It vanishes accidentally at $L=2$.  Consequently, a two-cell weighted lattice certificate is blind to the principal unscreened composition selector even though the all-$L$ charge certificate detects it for $L>2$.
...
\end{align}
All displayed terms are even, number conserving, finite range, and translation invariant.  Equation \eqref{eq:Hglobal-screened} is the composition-degenerate parent construction; \eqref{eq:Hglobal-unscreened} is the physical selector-plus-mixing construction.
...
For a family whose microscopic coefficients are held independent of $\lambda$, exact asymptotic composition degeneracy therefore requires the order-four residuals to vanish.  At any fixed design value of $\lambda$, however, \eqref{eq:tuning-tolerance} defines a nonzero tolerance tube around the matched point.  The unscreened corollary is the generic alternative: it requires no cancellation and retains the physical order-four selector explicitly.
...
\begin{corollary}[Unscreened selector-plus-mixing family]
...
\end{equation}
Thus the diagonal selector generally precedes the coherent order-$\lambda^6$ composition mixing.
\end{corollary}

\begin{remark}[What is and is not exact]
The exact global Feshbach map is not a sum of isolated-bond Schur complements, because neighboring controls share active sites.  The theorem instead states that overlap cannot renormalize the order-six multiplicative coefficient: all connected overlap terms begin at order eight.  The effective order-six energy density has relative error $O(\lambda^2)$ in the screened family.  In the unscreened family the leading composition physics is instead the order-four selector.
\end{remark}
...
\end{equation}
The $O(\lambda^8)$ statement includes the first two twist derivatives by Corollary~\ref{cor:peierls}.  The off-diagonal entry is therefore nonzero whenever $\bar\alpha>0$.  In the unscreened family the static order-four selector is twist independent and does not alter \eqref{eq:one-pair-curvature}, but it splits the composition levels before the order-six mixing is applied.
...
\end{equation}
with fractional design error $O(\varepsilon)+O(\lambda^2)$.  Exact asymptotic degeneracy is a tuned submanifold, whereas a finite experimental or numerical design has the nonzero tolerance tube quantified in \cref{sec:tuning}.  Without screening, the actual repulsive molecular shell generically produces the order-four diagonal selector \eqref{eq:unscreened-selector}, followed by the order-six coherent mixing.  The unscreened model is therefore not a failed version of the parent but a robust selector-plus-mixing problem with no cancellation requirement.
...
On a periodic lattice, a weighted hierarchy places that channel at order $\lambda^6$ and delays connected overlap, Kramers double-excitation, and residual block-coupling errors to order $\lambda^8$.  The screened family yields a composition-degenerate, translation-invariant, Peierls-covariant effective parent only on the matched submanifold, with the finite-$\lambda$ tolerance quantified in \eqref{eq:tuning-tolerance}.  The generic unscreened family retains the physically linked order-$\lambda^4$ diagonal selector and therefore realizes robust selector-plus-mixing physics.  The result completes a controlled multiorbital UV construction of the additional multiplicative escape channel, conditional on the base two-block QGN parent.  Material extraction and dynamical transport remain separate stages.
...
\end{equation}
Expanding $N_x^2$, using translation invariance and factorization between blocks, and summing $x$ gives \eqref{eq:charge-potential}.  The absence of off-diagonal composition matrix elements follows because $N_x$ commutes with both block charges.
